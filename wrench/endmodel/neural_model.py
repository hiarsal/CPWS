...
class MLPModel(BaseTorchClassModel):
    ...
    def fit(self,
        ...
        #                               batch_size=hyperparas['batch_size'], shuffle=True, num_workers=8, pin_memory=True)
        train_dataloader = DataLoader(TorchDataset(dataset_train, n_data=n_steps * hyperparas['batch_size']),
        ...
        model = MLP(
            ...
            n_hidden_layers=hyperparas['n_hidden_layers'],
        ...
        try:
            with trange(n_steps, desc="[TRAIN] MLP Classifier", unit="steps", disable=not verbose, ncols=150, position=0, leave=True) as pbar:
                model.train()
                step = 0
                print("before batch traning!")
                # print(len(train_dataloader))
                # print("traninloader[0]")
                # print(train_dataloader[0])
                for batch in train_dataloader:
                    # print("entering batch traning")
                    step += 1
                    optimizer.zero_grad()
                    outputs = model(batch)
                    batch_idx = batch['ids'].to(device)
                    target = y_train[batch_idx]
                    # print("test loss shape:", outputs.shape, target.shape)
