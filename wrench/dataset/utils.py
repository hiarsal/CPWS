...
def bert_text_extractor(data: List[Dict], device: torch.device = None, model_name: Optional[str] = 'bert-base-cased',
    ...
    def extractor(data: List[Dict]):
        ...
        model = AutoModel.from_pretrained(model_name, cache_dir='/data/rsl/CPWS/cache/').to(device) # slruan
        model.eval()
        tokenizer = AutoTokenizer.from_pretrained(model_name, cache_dir='/data/rsl/CPWS/cache/')  # e.g. 'bert-base-cased'
        text_features = []
...
def bert_relation_extractor(data: List[Dict], device: torch.device = None,
    ...
    def extractor(data: List[Dict]):
        ...
        model = AutoModel.from_pretrained(model_name, cache_dir='/data/rsl/CPWS/cache/').to(device)
        model.eval()
        tokenizer = AutoTokenizer.from_pretrained(model_name, cache_dir='/data/rsl/CPWS/cache/')  # e.g. 'bert-base-cased'
        text_features = []
