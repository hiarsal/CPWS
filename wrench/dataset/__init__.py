...
def load_dataset(data_home, dataset, dataset_ar, dataset_type=None, extract_feature=False, extract_fn=None, tf='train', ef='test',**kwargs):
    """
    @slruan: 
    1. add else to process NumericDataset
    2. add tf to process weak dataset, traning data 
    """
    ...
    dataset_path = Path(data_home) / dataset /dataset_ar

    train_data = dataset_class(path=dataset_path, split=tf)
    valid_data = dataset_class(path=dataset_path, split='valid')
    test_data = dataset_class(path=dataset_path, split=ef)
    # if extract_feature and (dataset_class != BaseSeqDataset):
    if extract_feature and (dataset_class == TextDataset):
        # print('test in __init__:', extract_fn)
        ...
        # print('test in __init__:', extract_fn)
        train_data.extract_feature(extract_fn=extract_fn, return_extractor=False, **kwargs)
        ...
        train_data.extract_feature(extract_fn=None, return_extractor=False, **kwargs)
        valid_data.extract_feature(extract_fn=None, return_extractor=False, **kwargs)
        test_data.extract_feature(extract_fn=None, return_extractor=False, **kwargs)
