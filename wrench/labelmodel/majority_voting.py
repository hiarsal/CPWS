...
class MajorityVoting(BaseLabelModel):
    ...
    def predict_proba(self, dataset: Union[BaseDataset, np.ndarray], noise: Optional[np.ndarray] = None,
                      weight: Optional[np.ndarray] = None, **kwargs: Any) -> np.ndarray:
        L = check_weak_labels(dataset)
        if weight is None:
            weight = np.ones(L.shape[1])
        if noise is None:
            noise = np.ones(L.shape[1])
        ...
        for i in range(n):
            ...
            for j in range(m):
                if L[i, j] != ABSTAIN:
                    ...
                    _noise  = np.random.choice([1, 0], p=[noise[j], 1-noise[j]])
                    if _noise==0:
                        L[i][j]=1-L[i][j]
                    counts[L[i, j]] += 1 * weight[j]
            # Y_p[i, :] = np.where(counts == max(counts), 1, 0)
            if counts.sum() == 0:
                counts += 1
            Y_p[i, :] = counts
